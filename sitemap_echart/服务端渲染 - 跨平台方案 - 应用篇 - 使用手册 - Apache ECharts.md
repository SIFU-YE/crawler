...
#### 图片的加载
node-canvas 提供了图片加载的`Image`实现，如果你在图表中使用了到了图片，我们可以使用`5.3.0`新增的`setPlatformAPI`接口来适配。
```
...
```
 id="chart-container" style="width:800px;height:600px"

 src="https://cdn.jsdelivr.net/npm/echarts/ssr/client/dist/index.min.js"
...
<!-- 方法一：使用 CDN -->
 src="https://cdn.jsdelivr.net/npm/echarts/ssr/client/dist/index.min.js"
<!-- 方法二：使用 NPM -->
 src="node_modules/echarts/ssr/client/dist/index.js"
```
...
大 | 大：没有引入的包就无法使用对应功能 | 小 | 首屏加载时间不敏感，对代码体积没有严格要求但是希望尽可能小，仅使用 ECharts 的一小部分功能，没有服务器资源  
