...
<?xml version="1.0" encoding="utf-8"?>
 xmlns="http://www.w3.org/2000/svg" version fill-rule xml:space
     name="named_rect" d="M 0,0 L 0,100 100,100 100,0 Z" fill 
     d="M 150,0 L 150,100 250,100 250,0 Z" fill 
...
```
  ...
  * 将这些元素包裹在一个具名的 中（如 `<g="name_a">`）中，这样只会显示一个标签，并且基于 的 `boundingRect` 定位。
...
第三，确定 SVG 的 `bounding rect`。`bounding rect` 由以下方法决定（它们的单位都是 SVG 内部元素的度量单位）：
  1. 如果设定了 geo.boundingCoords，则用它作 `bounding rect`。
  ...
  5. 如果设定了 geo.center 或 geo.zoom，则把上述 `1~4` 得到的 `bounding rect` 进行相应的 `transform`。
