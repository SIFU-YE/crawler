...
###  getColHeader 
  * getColHeader(columnIndexnumber)ITableColHeader
...
###  get 
  * getRow(rowIndexnumber)ITableRow
...
10.5.12
```
const row: ITableRow = portlet.getRow(0);
```

#### Parameters 参数
    * #####  rowIndex: number
行号，从0开始
####  Returns 返回值 ITableRow
行对象
...
10.5.12
```
const rows: ITableRow[] = portlet.getRows();
```
...
###  hideTableColumn 
  * hideTableColumn(colIndexnumber)void
...
```
const colIndex = 0;
portlet.hideTableColumn(colIndex);
```
...
单元格渲染handler，入参为ITableCell, ITableRow, rowIndex, columnIndex
####  Returns 返回值 void
