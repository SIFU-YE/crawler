# 菜鸟教程 -- 学的不仅是技术，更是梦想！
TypeScript 教程
TypeScript 教程  TypeScript 安装  TypeScript 基础语法  TypeScript 基本结构 TypeScript 基础类型  TypeScript 变量声明  TypeScript 运算符  TypeScript 条件语句  TypeScript 循环  TypeScript 函数  TypeScript Number  TypeScript String  TypeScript Array(数组)  TypeScript Map 对象 TypeScript 元组  TypeScript 联合类型  TypeScript 接口  TypeScript 类  TypeScript 对象  TypeScript 泛型 TypeScript 命名空间  TypeScript 模块  TypeScript 声明文件  TypeScript 测验 
# TypeScript 特性
...
interfacePerson{
  name:string;: number;
  greet():void;}classStudentimplementsPerson{
...
```
enumDirection{Up,Down,Left,Right,}let dir:Direction=Direction.Up;
```
...
```
classPerson{private name:string;protected age: number;public constructor(name:string, age: number){this.name = name;this.age = age;}}
```

### 8. **抽象类 (Abstract Classes)**
TypeScript 支持抽象类，抽象类不能直接实例化，需要由子类实现。抽象类适用于定义通用行为和抽象方法的类层次结构。
```
abstractclassAnimal{abstract makeSound():void;}classDogextendsAnimal{
  makeSound(){
    console.log("Woof!");}}
```
...
```
function identityTvalue:):{return value;}let num = identity<number>(42);
```

### 10. **模块和命名空间**
TypeScript 提供了基于 ES6 的模块系统，使用 `import` 和 `export` 导入和导出模块。此外，TypeScript 还支持命名空间（Namespace），用于组织代码和避免命名冲突。
```
// math.tsexportfunction add(a: number,: number): number {return+;}// main.tsimport{ add }from"./math";
console.log(add(2,3));
```
...
```
function printId(id:string| number){if(typeof==="string"){
...
interfaceTodo{
  title:string;
  description:string;}let partialTodo:PartialTodo={ title:"Learn TypeScript"};// 可选属性
```
